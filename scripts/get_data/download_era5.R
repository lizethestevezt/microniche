...
build_era5 <- function(site, overwrite = FALSE) {
  ...
  file_prefix <- paste0("era5_", site$Site)
  nc_file <- file.path(ERA5_DIR, paste0(file_prefix, ".nc"))
  ...
  req <- build_era5_request(xmin = site$lon_min, xmax = site$lon_max, 
                            ...
                            by_month = TRUE, outfile_name = file_prefix)
  
  request_era5(request = req, uid = credentials[3, 2], out_path = ERA5_DIR, overwrite = FALSE)
